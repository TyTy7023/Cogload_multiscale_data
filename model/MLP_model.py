...
class MLP:
    class MLP_Keras:
        ...
        def build(self, hp, shape):
            model = Sequential()
            num_hidden_layers = hp.Int('num_hidden_layers', min_value=2, max_value=5, step=1)
            model.add(Dense(units=hp.Int('units', min_value=32, max_value=128, step=32), activation="relu", input_shape=(shape,)))
        ...
        def tuner(self, directory, shape):
            return RandomSearch(
                hypermodel=self.build(shape=shape), 
